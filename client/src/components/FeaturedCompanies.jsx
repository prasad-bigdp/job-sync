...
function FeaturedCompanies() {
  return (
    <section className="bg-[#f1f5f9] py-10 sm:py-14 w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex gap-4 overflow-x-auto hide-scrollbar">
          {logos.map((logo, i) => (
            <div
              key={i}
              className="flex-shrink-0 w-36 h-20 bg-white flex justify-center items-center rounded-lg shadow-sm hover:shadow-md transition"
            >
              <img src={logo} alt={`company-${i}`} className="max-h-full object-contain" />
            </div>
          ))}
        </div>
      </div>
