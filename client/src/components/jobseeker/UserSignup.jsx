...
import {
  Box, Typography, Paper, useMediaQuery, InputAdornment, IconButton, CircularProgress,
  Button, Checkbox, FormControlLabel, TextField
} from '@mui/material';
import {
  Visibility, VisibilityOff, PersonOutline, EmailOutlined, LockOutlined, ArrowForward, PhoneAndroidOutlined
...
function UserSignup() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setLoading(true);
      try {
        await axios.post('http://127.0.0.1:5000/api/users', values);
        ...
        <Paper elevation={5} sx={{ maxWidth: 900, width: '100%', display: 'flex', flexDirection: { xs: 'column', md: 'row' }, overflow: 'hidden' }}>
          {!isSmallScreen && (
            <Box sx={{ width: '60%', p: 4, bgcolor: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <img src="https://res.cloudinary.com/dkg3qriqq/image/upload/v1747373698/4236127_ntd8xk.jpg" alt="Signup" style={{ maxWidth: '100%', maxHeight: '600px' }} />
            </Box>
          )}
